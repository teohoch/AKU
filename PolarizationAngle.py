	...
	def validate(self, correct):
		...
		try:
			...
			if x_angle_file == x_angle_calc and y_angle_file == y_angle_calc:
				...
				#If the option correct the polarization angle in xml file was selected.
				if correct:
					#Replace the wrong polarization angles with the calculated values.
					angles[0].setAttribute("PolXAngle", str(x_angle_calc))
					angles[0].setAttribute("PolYAngle", str(y_angle_calc))
					#Write correct polarization angles to the configuration file.
...
if __name__ == "__main__":
	usage = "usage: AngleValidator -p PATH_CONFIGURATION_FILES [-c]"
	parser = OptionParser(usage=usage)
	parser.add_option("-p", "--path", dest="path",
					  help="path to the FE configurations files.", metavar="PATH")
	parser.add_option("-c", "--correct", dest="correct",
					  help="correct the polarization angle in the configuration file.",
					  action="store_true", default=False)
	(options, args) = parser.parse_args()
	print('hello')
	# Check that the path is valid.
	print(options.path)
	print(os.path.isdir(options.path))
	if options.path is None or not os.path.isdir(options.path):
		print 'error'
		parser.error("input the path to the FE configurations file.")
	print 'no error'
	#Get all the configuration files for the Coldcarts.
	command = "grep ASSEMBLY.value=.ColdCart. %s/*" % options.path
	print(command)
	pipe = subprocess.Popen(command, stdout=subprocess.PIPE, shell=True)
	#state = pipe.wait()
	output, error = pipe.communicate()

	#Split the output in independent lines.
	output = output.split("\n")
	#The line format will be "file: <ASSEMBLY value="ColdCartx"/>", so split by ":".
	for element in output:
		xml_file = element.split(":")[0]
		print xml_file
		#Only the xml file will be analyzed.
		if ".xml" in xml_file:
			check = AngleValidator(xml_file)
			if options.correct and not check.validate(options.correct):
				print "The file %s was not using the correct polarization angles. File corrected." % xml_file
			if not options.correct and not check.validate(options.correct):
				print "The file %s is not using the correct polarization angles. If you want to correct it use the option -c" % xml_file